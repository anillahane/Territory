...
export default function TerritoryData() {
  ...
  const handleRefreshStoredData = useCallback(async () => {
    setRefreshingStoredData(true);
    try {
      const response = await api.refreshTerritoryDisplayData();
      setSuccess(
        `Stored branch coverage refreshed for ${Number(response?.refreshedBranches || 0)} branch(es).`
      );
      await loadTerritoryData();
    } catch (error: any) {
      setError(error?.message || 'Failed to refresh stored territory data.');
    } finally {
      setRefreshingStoredData(false);
    }
  }, [loadTerritoryData, setError, setSuccess]);
        ...
        >
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Stored branch coverage datasets computed from the latest branch and customer mappings.
              Refresh after batch uploads or mapping changes.
            </Typography>
          </Box>
          <Button
            variant="outlined"
            startIcon={refreshingStoredData ? <CircularProgress size={16} /> : <AutorenewIcon />}
            onClick={() => void handleRefreshStoredData()}
            disabled={loading || refreshingStoredData}
          >
            Refresh Stored Data
          </Button>
        </Stack>
        ...
        <Stack direction={{ xs: 'column', lg: 'row' }} spacing={2}>
          ...
          <FormControl fullWidth size="small">
            ...
            <Select
              ...
              disabled={loading || refreshingStoredData || branchOptionsLoading || branchOptions.length === 0}
            >
      ...
      <Stack direction={{ xs: 'column', xl: 'row' }} spacing={2}>
        <Paper sx={{ p: 2, borderRadius: 2, flex: 1 }}>
          <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap' }} useFlexGap>
            <Chip label="Data Source: Stored branch coverage" color="primary" variant="outlined" />
