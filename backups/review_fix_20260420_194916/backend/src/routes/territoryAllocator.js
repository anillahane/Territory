...
const runBucketAllocation = ({
  ...
  bucketKeys.forEach((bucketKey) => {
    ...
    if (neededEmployees <= normalizedThreshold) {
      ...
      metrics.push({
        ...
        accountTotal: bucketLoad
      });
      return;
    }

    const bucketCentroid = bucketPockets.reduce((accumulator, pocket) => ({
      lat: accumulator.lat + Number(pocket.centerLat),
      lng: accumulator.lng + Number(pocket.centerLng)
    }), { lat: 0, lng: 0 });

    const centroidLat = bucketCentroid.lat / bucketPockets.length;
    const centroidLng = bucketCentroid.lng / bucketPockets.length;
    const selectedEmployees = [...employees]
      ...
      .sort((left, right) => left.distanceToBucket - right.distanceToBucket)
      .slice(0, Math.min(neededEmployees, employees.length));
  ...
  return {
    ...
    summary: {
      ...
      assignedEmployees: employees.length,
      bucketsDirectAssigned: Object.values(bucketAssignments).length,
...
const resolveLatestMappingsJobId = async (client) => {
...
const handleTerritoryAllocate = async (req, res, client) => {
  try {
    ...
    const latestJobId = String(
      ...
      || req.body?.jobId
      || await resolveLatestMappingsJobId(client)
      || ''
    ).trim();
    ...
    if (!latestJobId) {
      throw new AppError(
        'No customer mappings are available. Upload and process a mapping file before territory allocation.',
        404,
        'NO_MAPPINGS'
      );
    }
