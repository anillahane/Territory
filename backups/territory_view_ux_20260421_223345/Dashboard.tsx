...
const buildTerritoryVisualizationCacheKey = (
  mode: TerritoryMode,
  branchIds: string[]
) => `${mode}|${branchIds.map((branchId) => String(branchId || '').trim()).join(',')}`;
...
export default function Dashboard({ territoryUiVariant = 'dashboard' }: DashboardProps) {
  ...
  const loadTerritoryVisualization = async (
    ...
    const cacheKey = buildTerritoryVisualizationCacheKey(mode, branchIds);
    ...
    try {
      ...
      decoratedBranches.features.forEach((feature) => {
      ...
      TERRITORY_MODE_OPTIONS.forEach((option) => {
        ...
        const prefetchKey = buildTerritoryVisualizationCacheKey(option.value, prefetchBranchIds);
        if (
