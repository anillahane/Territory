...
const { warmGridTileCache, isWarmupEnabledByEnv } = require('./services/GridService');
...
const startServer = () =>
  app.listen(PORT, () => {
    ...
    if (!isTestEnv && isWarmupEnabledByEnv()) {
      setImmediate(() => {
        warmGridTileCache().catch((error) => {
          logger.error(`Automatic grid warmup failed: ${error.message}`);
        });
      });
    }
  });
